  ...
  useEffect(() => {
    const fetchApplicantWorkData = async () => {
      try {
        ...
        // Fetch applicant work data based on application ID
        ...
        if (response.data && response.data.success) {
          ...
          if (data.work_experience) {
            try {
              workData = JSON.parse(data.work_experience);
            } catch {
              workData = [];
            }
          }
          ...
          if (Array.isArray(workData) && workData.length > 0) {
            const workRecords = workData.map((rec: any, idx: number) => ({
              id: idx + 1,
              work_id: rec.work_id,
              company: rec.company || "",
              role: rec.role || "",
          ...
          // Teaching Experience
          let teachingData: any[] = [];
          if (data.teaching_experience) {
            try {
              teachingData = JSON.parse(data.teaching_experience);
            } catch {
              teachingData = [];
            }
          }

          if (Array.isArray(teachingData) && teachingData.length > 0) {
            const teachingRecords = teachingData.map((rec: any, idx: number) => ({
              id: idx + 1,
          ...
          if (Array.isArray(skillsData) && skillsData.length > 0) {
            const skillRecords = skillsData.map((rec: any, idx: number) => ({
              id: idx + 1,
          ...
          if (data.languages) {
            try {
              languagesData = JSON.parse(data.languages);
            } catch {
              languagesData = [];
            }
          }
          if (Array.isArray(languagesData) && languagesData.length > 0) {
            const languageRecords = languagesData.map((rec: any, idx: number) => ({
              id: idx + 1,
